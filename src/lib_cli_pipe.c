...
*/
CLI_STATIC int cli_pipe_thread(IFXOS_ThreadParams_t *thr_params)
{
...
/** Pipe log
   ...
   \param[in] thr_params Thread parameters
*/
CLI_STATIC int cli_pipe_log_thread(
...
#if (CLI_OS_THREAD_PRIO != 0)
	IFXOS_ThreadPriorityModify(CLI_OS_THREAD_PRIO);
#endif
...
/** Init pipe
   ...
   \param[in] context CLI core context pointer
*/
int cli_pipe_init(
	struct cli_core_context_s *p_cli_core_ctx,
	int n_pipes,
	...
	struct cli_pipe_context_s **pp_cli_pipe_context)
{
	...
	if ((n_pipes == 0) || (n_pipes > MAX_CLI_PIPES)) {
		CLI_USR_ERR(LIB_CLI,
			("ERR CLI Pipe: init - invalid arg, n_pipes = %d (max = %d)" CLI_CRLF,
	...
	for (i = 0; i < n_pipes; i++) {
		...
		clios_snprintf(pipe_name, sizeof(pipe_name), CLI_PIPE_NAME_FORMAT "cmd", p_pipe_env->name_pipe, i);
		clios_pipe_create(pipe_name);
		clios_snprintf(pipe_name, sizeof(pipe_name), CLI_PIPE_NAME_FORMAT "ack", p_pipe_env->name_pipe, i);
		clios_pipe_create(pipe_name);
		clios_snprintf(p_pipe_env->name_pipe_event, sizeof(p_pipe_env->name_pipe_event),
			CLI_PIPE_NAME_FORMAT "event", p_pipe_name, i);
		clios_pipe_create(p_pipe_env->name_pipe_event);
		clios_snprintf(p_pipe_env->name_pipe_dump, sizeof(p_pipe_env->name_pipe_dump),
			CLI_PIPE_NAME_FORMAT "dump", p_pipe_name, i);
		clios_pipe_create(p_pipe_env->name_pipe_dump);

		clios_snprintf(pipe_name, sizeof(pipe_name), "tPipe_%d", i);
